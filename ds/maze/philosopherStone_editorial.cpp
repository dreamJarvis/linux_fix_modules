...
void add_rect(ll i) {
    ...
    if(itr!=rects_set.begin()) 
        prev_lh=prev(itr)->first;
    ...
    for( ; itr!=rects_set.end();){
        ...
        if(itr!=rects_set.end() && x<=prev_lh){ 
            clr_cnt[clr]-=x*y; 
            return; 
        }
        
        if(x<=itr->first){ 
            clr_cnt[clr]-=(itr->second)*(x-prev_lh); 
            rects_set.insert({x,y}); 
            return; 
        }
        else{
...
int main(){
    // to spped up the user input
    ios::sync_with_stdio(false);  

    // flushes the cout before cin accepts input  
    cin.tie(nullptr);
